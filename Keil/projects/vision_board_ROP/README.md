# PAC BTI Verification

## Intro

This project is a a template project to verify the PAC BTI on RT-Thread RA8D1 SoC with Arm Cortex-M85.

## Source Code

- source code: `/vision_board_vryPAC/src/hal_entry.c` 
...
1. Double click `mklinks.bat`，to create two links for `rt-thread`、`libraries` folders:
2. Build
- Double click **project.uvprojx** to open MDK project in Keil v5
- Build：
![](docs/picture/build.png)
3. Download

- Connect the Dev Board with PC through Dap-Link USB (USB-Debug)，then download.

![](docs/picture/download.png)

If necessary, change the size of RAM.

#### RT-Thread Studio

1. Double click `mklinks.bat`，to create two links for `rt-thread`、`libraries` folders
2. Open env in current folder，tap command `scons --target=eclipse`
3. Open RT-Thread Studio, File->Import current project
